...
// [[Rcpp::export]]
mat mvrnormArma(int k, vec mu, mat sigma) { //k is nRep
  int ncols = sigma.n_cols;
  mat Y = randn(k, ncols);
  return repmat(mu, 1, k).t() + Y * chol(sigma);
}

// [[Rcpp::export]]
mat rwishart(unsigned int df, const mat& S){
...
public:
  ...
  void updateGeneMbar(int n){
    // New update function with covariance matrices
    mat Sigma_G_1 = Sigma_G.i();
    double gamma2  = as_scalar((gammaMbar.t())*gammaMbar);
    mat Sigma_n_G = (Sigma_G_1 + (pow(sigma,-2)*gamma2)).i();
    vec B(nGene); // M_n * Omega
    vec methy_n = methy.row(n).t(); // do we need to transpose it like this?

    for(int k=0; k<nGene; k++){
      if(JkAll(k).size()==1)
        B(k) = as_scalar(methy_n(JkAll(k)(0)) * omega(JkAll(k)(0)));
      else
        B(k) = as_scalar(methy_n.subvec(JkAll(k)(0), JkAll(k)(JkAll(k).size()-1)).t() * omega.subvec(JkAll(k)(0), JkAll(k)(JkAll(k).size()-1)));
    }
    double A = as_scalar(Y(n) - C.row(n)*gammaC - geneM.row(n)* gammaM - B.t()*gammaMbar);

    vec mu(nGene);
    mu = Sigma_n_G * (Sigma_G_1*B + gammaMbar * A * pow(sigma,-2)); // KxK * Kx1 + kx1
    geneMbar.row(n) = mvrnormArma(1, mu, Sigma_n_G);
  }
  ...
  void updateMethyMiss(int n){

    mat Sigma_G_1 = (Sigma_G).i();
    mat Sigma_M_1 = (Sigma_M).i();
    vec methy_n = methy.row(n).t();
    vec MOmega(nGene);

    for(int k=0; k<nGene; k++){
      if(JkAll(k).size()==1)
        MOmega(k) = as_scalar(methy_n(JkAll(k)(0)) * omega(JkAll(k)(0)));
      else
        MOmega(k) = as_scalar(methy_n.subvec(JkAll(k)(0), JkAll(k)(JkAll(k).size()-1)).t() * omega.subvec(JkAll(k)(0), JkAll(k)(JkAll(k).size()-1)));
    }

    double A = as_scalar(Y(n) - C.row(n)*gammaC - gene.row(n)*gammaMbar);
    mat Sigma_n_M = (Sigma_M_1 + as_scalar(omega.t()*Sigma_M_1*omega) + (omega*(gammaMbar - gammaM).t())*(omega*(gammaMbar - gammaM).t()).t()).i();
    vec B = Sigma_G_1*gene.row(n).t();
    vec mu(nMethy);
    vec tmpvec(nMethy); //Mx1 substitution for omega * ( A*(gammaMbar-gammaM)*pow(sigma, -2) + Sigma_G_1*B ) where omega is a matrix.
    vec tmpvec_sub = ( A*(gammaMbar-gammaM)*pow(sigma, -2) + Sigma_G_1*B );

    for(int k=0; k<nGene; k++){
      if(JkAll(k).size()==1)
        tmpvec(JkAll(k)(0)) = as_scalar(tmpvec_sub(k) * omega(JkAll(k)(0)));
      else
        tmpvec.subvec(JkAll(k)(0), JkAll(k)(JkAll(k).size()-1)) = tmpvec_sub(k) * omega.subvec(JkAll(k)(0), JkAll(k)(JkAll(k).size()-1))  ;
    }
    mu = Sigma_n_M * tmpvec;
    methy.row(n) = mvrnormArma(1, mu, Sigma_n_M);
  }
  ...
  void updateIOmega(int j, int itr){
    int k = mapMethy(j);
    if(!isfinite(omega(j)))
      cout << "Omega for gene" << k << " fails to converge"<<endl;
  ...
  void updateSigma_M (int itr){
    mat Q_M  = zeros<mat>(nMethy, nMethy);
    Q_M = riwishart(2*nMethy+1, eye<mat>(nMethy, nMethy));
    Sigma_M = Q_M;
  }

  void updateSigma_G (int itr){
    mat Q_G  = zeros<mat>(nGene, nGene);
    Q_G = riwishart(2*nMethy+1, (eye<mat>(nGene, nGene)+geneMbar.t()*geneMbar));
    Sigma_G = diagmat(sigmak) *Q_G* diagmat(sigmak);
  }
      ...
      for(int itr=0; itr<nItr; itr++){
        ...
        if(nMissMethy>0){
          for(int l=0; l<nMissMethy; l++){
            updateMethyMiss(missMethyIdx(l));
          }
        }
...
Rcpp::List FBMcpp(const Rcpp::List& data, int nItr, const int seed, const string method){
  ...
  parm.Sigma_M = eye<mat>(nMethy, nMethy);
  parm.Q_M = parm.Sigma_M;
  parm.Q_G = eye<mat>(nGene, nGene);
  parm.Sigma_G = diagmat(parm.sigmak) *parm.Q_G* diagmat(parm.sigmak);
